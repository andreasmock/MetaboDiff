...
```{r}
metadata(met)$res_pca = PCA(t(assays(met)[["norm_imputed"]]), 
              scale.unit = TRUE,
              graph=FALSE)
```
...
```{r}
fviz_screeplot(metadata(met)$res_pca , addlabels=TRUE)
```

PCA plot.

```{r}
fviz_pca_ind(metadata(met)$res_pca,
             label="none",
...
plotDendroAndColors(metadata(met)$tree, 
...
```{r}
par(mar=c(2,2,2,2))
plot.phylo(as.phylo(metadata(met)$METree),
           type = 'fan',
