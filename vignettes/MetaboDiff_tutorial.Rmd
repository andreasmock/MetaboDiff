...
ggplot(mdata, 
       mapping=aes(x=colname,
                          y=value,
                          fill=tumor_normal)) + 
    geom_boxplot() + ylim(0, 75000) + xlab("") +
    theme(axis.text.x=element_blank()) + ylab("raw abundance value") + scale_fill_manual(values=c("darkseagreen","dodgerblue"))
```

# Normalization

```{r}
meanSdPlot(assay(met))
```
...
```{r,eval=F}
autoplot(prcomp(assays(met)[["norm_imputed"]]))
```
