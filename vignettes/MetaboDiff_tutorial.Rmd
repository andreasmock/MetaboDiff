...
WGCNA of the 472 TCGA melanoma samples revealed 41 co-expression modules. All genes that are not significantly co-expressed within a module are summarized in an additional module 0 for further analysis.
...
plotDendroAndColors(metadata(met)$tree, metadata(met)$module_colors, 'Module colors', dendroLabels = FALSE, hang = 0.03,
...
MEs = moduleEigengenes(metadata(met)$diss_matrix, colors = metadata(met)$module_colors, excludeGrey = FALSE)$eigengenes
...
par(mar=c(2,2,2,2))
plot.phylo(as.phylo(METree),
           type = 'fan',
           show.tip.label = FALSE, 
           main='')
tiplabels(frame = 'circle',
          col='black', 
