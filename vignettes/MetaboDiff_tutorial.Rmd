...
author:
- name: Andreas Mock
  affiliation: University Hospital, German Cancer Research Center (DKFZ) and National Center for Tumor Diseases (NCT), Heidelberg
...
If asked, install the package from source. Alternatively you might need to download the package from the [developer](https://labs.genetics.ucla.edu/horvath/CoexpressionNetwork/Rpackages/WGCNA/)  
...
The imputed data of the example study set displays a cluster of 5 samples (cluster 1) with in average lower relative metabolite measurements. Due to the lack of batch information, this cannot be investigted further at this time. To demonstrate, how a cluster can be removed, we apply the function `remove_cluster` to remove cluster 1:
...
Variance stabilizing normalization (vsn) is used ensure that the variance remains nearly constant over the measured spectrum[^6].
...
A principal component analysis is performed and saved in the metadata slot by
...
Figure \@ref(fig:var) visualised the percentage of explained variances for the first ten principal components i.e. dimensions.

```{r var, fig.cap="Variance explained by principal components. Barplot for principal components one to ten are displayed.", fig.small=TRUE}
factoextra::fviz_screeplot(metadata(met)$res_pca , addlabels=TRUE)
```

In the example data, principal component 1 explains 17.1% and principal component 2 9.4% of the variance.
The following function creates the PCA plot for the grouping of interest (figure \@ref(fig:pcaplot)).

```{r pcaplot, fig.cap="PCA plot. The samples are colored according to the grouping of interest."}
...
The comparative analysis is visualized by means of a volcano plot (figure \@ref(fig:volcano1)).
...
As expected, no metabolite was significantly different between the randomly assigned grouping male vs. female after multiple testing with a cutoff of p<0.05 (figure \@ref(fig:volcano2)).
...
```{r varbox, fig.width=8, fig.height=9, fig.cap="Pathway-wide distribution of normalized relative metabolite measurment. The pathway annotation from the Small Molecular Pathway Database (SMPDB) was used."}
...
For detailed rational of this approach, please see Zhang and Horvath[^10]. For metabolic networks, we identified that a beta value of 3 was the lowest power for which the scale-free topology of the topology was met.
...
```{r eigengenes, fig.cap="Hierarchical of metabolic correlation modules as revealed by the clustering of module eigengenes. Each node represents a metabolic correlation module."}
par(mar=c(2,2,2,2))
...
```{r naming, fig.cap="Hierarchy of metabolic correlation modules. Modules are named named according to the most abundant pathway annotation in a module. Module 0 comprises the two metabolites without a significant interaction."}
...
```{r MOI_plot, results="asis", eval=FALSE}
MOI_plot(met,
         group_factor="tumor_normal",
         MOI=2)
```
