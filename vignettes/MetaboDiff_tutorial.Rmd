...
The `r Githubpkg("andreasmock/MetaboDiff")` R package can be installed via Github and requires R version >= 3.4. 
...
Alternatively you might need to download the package from the [developer](https://labs.genetics.ucla.edu/horvath/CoexpressionNetwork/Rpackages/WGCNA/)  
...
If you encounter problems installing `WGCNA`, please refer to the [developer page](https://labs.genetics.ucla.edu/horvath/CoexpressionNetwork/Rpackages/WGCNA/). 
...
```{r quality, message=FALSE, warning=FALSE, fig.cap="Quality control plot. Boxplot displaying the distribution of (A) raw, (B) imputed (C) normalized and (D) imputed and normalized metabolite measurements for all samples of the study set. Boxplot are colored according to the grouping of interest, i.e. tumor vs normal.", fig.width=10, fig.height=10}
quality_plot(met,
             group_factor="tumor_normal",
             label_colors=c("darkseagreen","dodgerblue"))
```
...
In the example data, principal component 1 explains 17.1% and principal component 2 9.4% of variance.
The following function creates the PCA plot for the grouping of interest.

```{r pcaplot, fig.cap="PCA plot. The samples are colored according to the grouping of interest.", fig.small=TRUE}
...
```{r var_boxplot, fig.width=8, fig.height=9}
...
```{r MS_plot}
png("plots/MS_plot.png", width=10, height=5, res=300, units = "in")
MS_plot(met,
        group_factor="tumor_normal")
dev.off()
```
